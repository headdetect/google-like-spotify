...
gmusic.init({email: config.email, password: config.password}, function(err) {
  ...
  gmusic.getPlayListEntries(function(err, data) {

      var songs = data.data.items.filter(entry => {

        // Make sure it belongs in our selected playlist
        // Make sure its a song in the playlist
        // Make sure its not already in the other playlist
        return entry.playlistId == "def948ca-ce6a-4a06-920e-2f4b58f0fdd9";
      }).sort((a, b) => a.absolutePosition - b.absolutePosition);

      var lastSong = songs[songs.length - 1];
      var supposedToBeLast = songs.findIndex(function(entry) { return entry.track && entry.track.title == "All Of The Lights (Explicit)" }); // All of the lights CID //
